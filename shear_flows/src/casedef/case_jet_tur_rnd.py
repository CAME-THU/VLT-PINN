"""Case 4 (turbulent round jet): Parameters, reference solution, and governing equations."""
import math as mt
# import numpy as np
# import torch
...
class Case:
    def __init__(self, args):
        ...
        self.delta = 6e-2
        self.K = 1e-3  # K = J / rho, kinetic momentum flux
        self.temp_flux = 1e-2
        self.alpha = 0.017
        self.nut = self.alpha * (3 * self.K / mt.pi) ** 0.5
        self.Pr_t = 0.84

        eta_half_umax = (2 ** 0.5 - 1) ** 0.5
        width_half_umax = 2 * 8 * self.alpha * self.delta * eta_half_umax
        umax = (0.125 / self.alpha) * (3 * self.K / mt.pi) ** 0.5 * self.delta ** (-1)
        self.Re_width = umax * width_half_umax / self.nu

    def func_eta(self, x, y):
        return y / (8 * self.alpha * x)
    ...
    def func_u_ind(self, x, y):
        ...
        u_max = (0.125 / self.alpha) * (3 * self.K / mt.pi) ** 0.5 * x_ ** (-1)  # same as last line
    ...
    def func_T_ind(self, x, y):
        x_ = x + self.delta
        eta = self.func_eta(x_, y)
        T_max = (1 + 2 * self.Pr_t) * self.temp_flux / (8 * self.alpha * (3 * mt.pi * self.K) ** 0.5 * x_)
        return T_max * (1 + eta ** 2) ** (-2 * self.Pr_t)

    def func_psi_ind(self, x, y):
        x_ = x + self.delta
        eta = self.func_eta(x_, y)
        return self.nut * x_ * 4 * eta / (1 + eta ** 2)
    ...
    def pde_uv(self, xy_s, uv_s):
        ...
        momentum_x = ys_ * (us * us_xs + cy * vs * us_ys) - cyy * self.nut * (us_ys + ys_ * us_ysys)
        continuity *= k  # / y_r
        momentum_x *= k  # / y_r
    ...
    def pde_uvT(self, xy_s, uvT_s):
        ...
        continuity = ys_ * (us_xs + cy * vs_ys) + cy * vs
        momentum_x = ys_ * (us * us_xs + cy * vs * us_ys) - cyy * self.nut * (us_ys + ys_ * us_ysys)
        energy = ys_ * (us * Ts_xs + cy * vs * Ts_ys) - cyy * (self.nut / self.Pr_t) * (Ts_ys + ys_ * Ts_ysys)
        continuity *= k  # / y_r
        momentum_x *= k  # / y_r
        energy *= k  # / y_r
