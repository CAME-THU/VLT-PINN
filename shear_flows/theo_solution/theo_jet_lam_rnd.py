...
delta = 2e-2
K = 1e-7  # K = J / rho, kinetic momentum flux
temp_flux = 1e-4
Pr = 0.71

eta_half_umax = (2 ** 0.5 - 1) ** 0.5
width_half_umax = 2 * 8 * (mt.pi / (3 * K)) ** 0.5 * nu * delta * eta_half_umax
umax = 0.375 * K / (mt.pi * nu * delta)
Re_width = umax * width_half_umax / nu

save_dir = f"./results/jet_lam_round/"
...
# define functions
def func_eta(x, y):
    return 0.125 * (3 * K / mt.pi) ** 0.5 / nu * (y / x)


def func_u(x, y):
    x_ = x + delta
    eta = func_eta(x_, y)
    u_max = 0.375 * K / (mt.pi * nu * x_)
...
def func_T(x, y):
    x_ = x + delta
    eta = func_eta(x_, y)
    T_max = (1 + 2 * Pr) * temp_flux / (8 * mt.pi * nu * x_)
    return T_max * (1 + eta ** 2) ** (-2 * Pr)


def func_psi(x, y):
    x_ = x + delta
    eta = func_eta(x_, y)
    return nu * x_ * 4 * eta / (1 + eta ** 2)
