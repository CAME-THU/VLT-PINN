...
save_dir = f"./results/wake_lam_plane/"
...
def func_u(x, y):
    ...
    return u_far * (1 - c * (L / x_) ** 0.5 * f)


def func_v(x, y):
    x_ = x + delta
    f = mt.e ** (-0.25 * u_far / nu * y ** 2 / x_)
    return u_far * (-0.5) * c * (L ** 0.5 * y / x_ ** 1.5) * f


def func_psi(x, y):
    x_ = x + delta
    eta = y * (0.25 * u_far / (nu * x_)) ** 0.5
    return u_far * y - c * (mt.pi * nu * L * u_far) ** 0.5 * ss.erf(eta)
